...
async function getEvents() {
	...
	for (let i = 0; i < myEvents.length; i++) {
		document.querySelector("#container").innerHTML += getCardHtml(myEvents[i].name, myEvents[i].day, myEvents[i].text, myEvents[i].src, myEvents[i].id, mainId[i].id);
	}
...
async function deleteCard(event, id) {
	...
	const link = 'http://localhost:3000/userEvents/' + id;
	console.log(link);
...
function getCardHtml(name, day, text, src, id, mainId) {
	return `
			<div class="card me-4" data-card-id=${id} style="width: 18rem;">
				...
				<div class="card-body">
					...
					<form onsubmit="deleteCard(event, ${mainId})">
