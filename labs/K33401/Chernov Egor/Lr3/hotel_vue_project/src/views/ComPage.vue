      ...
      <div class="container col-8 justify-content-center py-4">
        ...
        <ul class="navbar-nav p-3">
          <li class="nav-item" v-for="comment in comments" :key="comment.id">
            <com-item v-if="comment.user_com.id.toString() === idUser.toString()" :username="comment.user_com.username" :rating_com="comment.rating_com" :review_com="comment.review_com" :check_in_com="comment.check_in_com" :check_out_com="comment.check_out_com" />
          </li>
        </ul>
      </div>
    </div>
